...
3) 准备好 BUG 表（用 emmy-lark；已有表用链接里解析的 token，没有就先自建）：
   ...
   - 自检 + 自动补全字段（已有表也走这步）：先 `emmy-lark base +field-list --base-token <t> --table-id <tbl>` 看现有字段，缺的就【自动建】(纯新增、低危)：
     ...
     - **所属模块（配了不止一个仓时【必建】）**：`select`，选项=各模块名（和 `repos` 的 key 一致，如 前端/服务端）：`--json '{"name":"所属模块","type":"select","options":[{"name":"前端"},{"name":"服务端"}]}'`。worker 靠它路由到对应仓——**没这字段或没填，它会对每条 BUG 都卡住问"改哪个仓"**。只有一个仓就别建。
