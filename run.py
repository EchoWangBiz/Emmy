...
3) 准备好 BUG 表（用 emmy-lark；已有表用链接里解析的 token，没有就先自建）：
   ...
   - 自检 + 自动补全字段（已有表也走这步）：先 `emmy-lark base +field-list --base-token <t> --table-id <tbl>` 看现有字段。
     - **worker 硬必需**：状态、修复分支/PR、AI备注、待确认问题；通知/登记建议有：提问人、附件/截图、问题编号。缺这些才尝试【自动建】(纯新增、低危)。
     - **业务描述字段走兼容，不硬补**：`问题内容` 可替代 `问题摘要 + 复现/期望/实际`；`问题类型` 可替代 `类型`；`所属模块` 可替代部分分类；`负责人` 可没有。已有这些兼容字段时，不要为了标准模板再建 `问题摘要/复现/负责人/类型`，也不要因缺 `base:field:create` 卡初始化。
     - 文本字段（提问人 / 修复分支/PR / AI备注 / 待确认问题 / 提问人答复）：`--json '{"name":"AI备注","type":"text"}'`
   ...
   - 必须齐：状态(至少含待修复/修复中/待人工确认/待发布)、修复分支/PR、AI备注、待确认问题；建议齐：提问人、附件/截图、问题编号。业务描述字段只要有 `问题内容` 或 `问题摘要/复现/期望/实际` 这类可承载内容的字段即可。
